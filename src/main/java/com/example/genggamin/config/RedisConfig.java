...
/** Redis Configuration Class Mengkonfigurasi Redis untuk caching data user dan role */
@Configuration
@EnableCaching
public class RedisConfig {
  ...
  /**
   * Configure Cache Manager untuk Spring Cache Abstraction Menggunakan Redis sebagai cache provider
   */
  @Bean
  public RedisCacheManager cacheManager(RedisConnectionFactory factory) {
    ...
    RedisCacheConfiguration userConfig =
        defaultConfig.entryTtl(Duration.ofMinutes(15)); // User cache 15 menit

    RedisCacheConfiguration roleConfig =
        defaultConfig.entryTtl(Duration.ofMinutes(30)); // Role cache 30 menit (jarang berubah)
  ...
  /**
   * ObjectMapper configuration untuk serialization/deserialization REDIS ONLY Mendukung Java Time
   * API dan polymorphic types NOTE: ObjectMapper ini HANYA untuk Redis, tidak untuk HTTP
   * request/response
   */
